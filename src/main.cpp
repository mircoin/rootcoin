...
uint256 hashGenesisBlock("0xa565f5ff4c2aed566e6f7d3957eda4ac2a9a72a127462b141a1307fb95b2fa3b");
...
{
    // Rootcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
{
    // Rootcoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
	    ...
	    assert(block.hashMerkleRoot == uint256("0x78337181b76de0792814d376d60ba98f22fd599cf67ef7068997f93be1cc7dd7"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xd4, 0x7a, 0xa7, 0xeb };
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("RootcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("RootcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static RootcoinMiner(CWallet *pwallet)
{
    printf("RootcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("Rootcoin-miner");
    ...
    try { loop {
        ...
        printf("Running RootcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("RootcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&RootcoinMiner, pwallet));
}
